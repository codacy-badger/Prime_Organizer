	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'tb_requerimento' => array('Requerimento de Vagas', 'Requerimento de vagas por parceiros', 'table.gif', 'RH')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'tb_vaga' => "`tb_vaga`.`id` as 'id', IF(    CHAR_LENGTH(`tb_requerimento1`.`id`), CONCAT_WS('',   `tb_requerimento1`.`id`), '') as 'requerimento_id', `tb_vaga`.`int_vaga_numero` as 'int_vaga_numero', IF(    CHAR_LENGTH(if(`tb_requerimento2`.`dta_abertura`,date_format(`tb_requerimento2`.`dta_abertura`,'%d/%m/%Y'),'')), CONCAT_WS('',   if(`tb_requerimento2`.`dta_abertura`,date_format(`tb_requerimento2`.`dta_abertura`,'%d/%m/%Y'),'')), '') as 'dta_abertura', if(`tb_vaga`.`dta_fechamento`,date_format(`tb_vaga`.`dta_fechamento`,'%d/%m/%Y'),'') as 'dta_fechamento', IF(    CHAR_LENGTH(`tb_empresa1`.`str_nome_fantasia`), CONCAT_WS('',   `tb_empresa1`.`str_nome_fantasia`), '') as 'empresa_id', IF(    CHAR_LENGTH(`tb_alocacao1`.`str_nome`), CONCAT_WS('',   `tb_alocacao1`.`str_nome`), '') as 'str_alocacao', IF(    CHAR_LENGTH(`tb_recrutador1`.`str_nome`), CONCAT_WS('',   `tb_recrutador1`.`str_nome`), '') as 'recrutador_id', `tb_vaga`.`str_posicao` as 'str_posicao', `tb_vaga`.`str_prioridade` as 'str_prioridade', `tb_vaga`.`str_status` as 'str_status', if(`tb_vaga`.`dta_previsao_fechamento`,date_format(`tb_vaga`.`dta_previsao_fechamento`,'%d/%m/%Y'),'') as 'dta_previsao_fechamento', `tb_vaga`.`str_obs` as 'str_obs', if(`tb_vaga`.`dta_inicio`,date_format(`tb_vaga`.`dta_inicio`,'%d/%m/%Y'),'') as 'dta_inicio', `tb_vaga`.`str_contratado_nome` as 'str_contratado_nome'",
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'tb_requerimento' => "`tb_requerimento` LEFT JOIN `tb_empresa` as tb_empresa1 ON `tb_empresa1`.`id`=`tb_requerimento`.`empresa_id` LEFT JOIN `tb_contato` as tb_contato1 ON `tb_contato1`.`id`=`tb_requerimento`.`contato_id` "
		);
