		...
		$pcConfig = array(
			'tb_vaga' => array(   
				'empresa_id' => array(   
					...
					'forced-where' => '',
					'display-fields' => array(1 => 'REQ', 2 => 'Vaga', 3 => 'Data de abertura', 5 => 'Data de fechamento', 6 => 'Status', 7 => 'Detalhes do Status', 8 => 'Recrutador', 9 => 'Prioridade', 10 => 'Posi&#231;&#227;o', 11 => 'Empresa', 13 => 'Canal de Fechamento', 14 => 'Observa&#231;&#245;es'),
					'display-field-names' => array(1 => 'requerimento_id', 2 => 'int_vaga_numero', 3 => 'dta_abertura', 5 => 'dta_fechamento', 6 => 'str_status', 7 => 'str_detalhe_status', 8 => 'recrutador_id', 9 => 'str_prioridade', 10 => 'str_posicao', 11 => 'empresa_id', 13 => 'canal_fechamento', 14 => 'str_obs'),
					'sortable-fields' => array(0 => '`tb_vaga`.`id`', 1 => '`tb_requerimento1`.`id`', 2 => '`tb_vaga`.`int_vaga_numero`', 3 => '`tb_vaga`.`dta_abertura`', 4 => '`tb_vaga`.`dta_previsao_fechamento`', 5 => '`tb_vaga`.`dta_fechamento`', 6 => 7, 7 => 8, 8 => 9, 9 => 10, 10 => 11, 11 => '`tb_empresa1`.`str_nome_fantasia`', 12 => 13, 13 => 14, 14 => 15, 15 => '`tb_vaga`.`dta_inicio`'),
					'records-per-page' => 10,
					'default-sort-by' => 3,
			...
			'tb_entrevista' => array(   
				'empresa_id' => array(   
					...
					'query' => "SELECT `tb_entrevista`.`id` as 'id', IF(    CHAR_LENGTH(`tb_empresa1`.`str_nome_fantasia`), CONCAT_WS('',   `tb_empresa1`.`str_nome_fantasia`), '') as 'empresa_id', IF(    CHAR_LENGTH(`tb_vaga1`.`str_posicao`) || CHAR_LENGTH(`tb_vaga1`.`str_alocacao`), CONCAT_WS('',   `tb_vaga1`.`str_posicao`, ' - ', `tb_vaga1`.`str_alocacao`), '') as 'vaga_id', IF(    CHAR_LENGTH(`tb_recrutador1`.`str_nome`), CONCAT_WS('',   `tb_recrutador1`.`str_nome`), '') as 'recrutador_id', if(`tb_entrevista`.`dta`,date_format(`tb_entrevista`.`dta`,'%d/%m/%Y'),'') as 'dta', `tb_entrevista`.`int_qtd_contatos` as 'int_qtd_contatos', `tb_entrevista`.`int_qtd_entrevistas` as 'int_qtd_entrevistas', `tb_entrevista`.`int_qtd_encaminhamentos` as 'int_qtd_encaminhamentos' FROM `tb_entrevista` LEFT JOIN `tb_empresa` as tb_empresa1 ON `tb_empresa1`.`id`=`tb_entrevista`.`empresa_id` LEFT JOIN `tb_vaga` as tb_vaga1 ON `tb_vaga1`.`id`=`tb_entrevista`.`vaga_id` LEFT JOIN `tb_recrutador` as tb_recrutador1 ON `tb_recrutador1`.`id`=`tb_entrevista`.`recrutador_id` "
				)
			),
