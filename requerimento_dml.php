...
// Data functions (insert, update, delete, form) for table requerimento

// This script and data application were generated by AppGini 5.72
// Download AppGini for free from https://bigprof.com/appgini/download/

function requerimento_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('requerimento');
	if(!$arrPerm[1]){
		return false;
	}
	...
	// hook: requerimento_before_insert
	if(function_exists('requerimento_before_insert')){
		$args=array();
		if(!requerimento_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `requerimento` set       `str_status`=' . (($data['str_status'] !== '' && $data['str_status'] !== NULL) ? "'{$data['str_status']}'" : 'NULL') . ', `str_posicao`=' . (($data['str_posicao'] !== '' && $data['str_posicao'] !== NULL) ? "'{$data['str_posicao']}'" : 'NULL') . ', `int_n_vagas`=' . (($data['int_n_vagas'] !== '' && $data['int_n_vagas'] !== NULL) ? "'{$data['int_n_vagas']}'" : 'NULL') . ', `str_reposicao`=' . (($data['str_reposicao'] !== '' && $data['str_reposicao'] !== NULL) ? "'{$data['str_reposicao']}'" : 'NULL') . ', `str_recurso`=' . (($data['str_recurso'] !== '' && $data['str_recurso'] !== NULL) ? "'{$data['str_recurso']}'" : 'NULL') . ', `time_horario_entrada`=' . (($data['time_horario_entrada'] !== '' && $data['time_horario_entrada'] !== NULL) ? "'{$data['time_horario_entrada']}'" : 'NULL') . ', `time_horario_saida`=' . (($data['time_horario_saida'] !== '' && $data['time_horario_saida'] !== NULL) ? "'{$data['time_horario_saida']}'" : 'NULL') . ', `empresa_id`=' . (($data['empresa_id'] !== '' && $data['empresa_id'] !== NULL) ? "'{$data['empresa_id']}'" : 'NULL') . ', `contato_id`=' . (($data['contato_id'] !== '' && $data['contato_id'] !== NULL) ? "'{$data['contato_id']}'" : 'NULL') . ', `str_gestor`=' . (($data['str_gestor'] !== '' && $data['str_gestor'] !== NULL) ? "'{$data['str_gestor']}'" : 'NULL') . ', `str_telefone`=' . (($data['str_telefone'] !== '' && $data['str_telefone'] !== NULL) ? "'{$data['str_telefone']}'" : 'NULL') . ', `str_email`=' . (($data['str_email'] !== '' && $data['str_email'] !== NULL) ? "'{$data['str_email']}'" : 'NULL') . ', `float_salario`=' . (($data['float_salario'] !== '' && $data['float_salario'] !== NULL) ? "'{$data['float_salario']}'" : 'NULL') . ', `int_maquinas`=' . (($data['int_maquinas'] !== '' && $data['int_maquinas'] !== NULL) ? "'{$data['int_maquinas']}'" : 'NULL') . ', `str_beneficios`=' . (($data['str_beneficios'] !== '' && $data['str_beneficios'] !== NULL) ? "'{$data['str_beneficios']}'" : 'NULL') . ', `bool_abertura`=' . (($data['bool_abertura'] !== '' && $data['bool_abertura'] !== NULL) ? "'{$data['bool_abertura']}'" : 'NULL') . ', `data_indicacao`=' . (($data['data_indicacao'] !== '' && $data['data_indicacao'] !== NULL) ? "'{$data['data_indicacao']}'" : 'NULL') . ', `str_descricao`=' . (($data['str_descricao'] !== '' && $data['str_descricao'] !== NULL) ? "'{$data['str_descricao']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"requerimento_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: requerimento_after_insert
	if(function_exists('requerimento_after_insert')){
		$res = sql("select * from `requerimento` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!requerimento_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('requerimento', $recID, getLoggedMemberID());

	return $recID;
}

function requerimento_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('requerimento');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='requerimento' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='requerimento' and pkValue='$selected_id'");
	...
	// hook: requerimento_before_delete
	if(function_exists('requerimento_before_delete')){
		$args=array();
		if(!requerimento_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `requerimento` where `id`='$selected_id'", $eo);

	// hook: requerimento_after_delete
	if(function_exists('requerimento_after_delete')){
		$args=array();
		requerimento_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='requerimento' and pkValue='$selected_id'", $eo);
}

function requerimento_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('requerimento');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='requerimento' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='requerimento' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: requerimento_before_update
	if(function_exists('requerimento_before_update')){
		$args=array();
		if(!requerimento_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `requerimento` set       `str_status`=' . (($data['str_status'] !== '' && $data['str_status'] !== NULL) ? "'{$data['str_status']}'" : 'NULL') . ', `str_posicao`=' . (($data['str_posicao'] !== '' && $data['str_posicao'] !== NULL) ? "'{$data['str_posicao']}'" : 'NULL') . ', `int_n_vagas`=' . (($data['int_n_vagas'] !== '' && $data['int_n_vagas'] !== NULL) ? "'{$data['int_n_vagas']}'" : 'NULL') . ', `str_reposicao`=' . (($data['str_reposicao'] !== '' && $data['str_reposicao'] !== NULL) ? "'{$data['str_reposicao']}'" : 'NULL') . ', `str_recurso`=' . (($data['str_recurso'] !== '' && $data['str_recurso'] !== NULL) ? "'{$data['str_recurso']}'" : 'NULL') . ', `time_horario_entrada`=' . (($data['time_horario_entrada'] !== '' && $data['time_horario_entrada'] !== NULL) ? "'{$data['time_horario_entrada']}'" : 'NULL') . ', `time_horario_saida`=' . (($data['time_horario_saida'] !== '' && $data['time_horario_saida'] !== NULL) ? "'{$data['time_horario_saida']}'" : 'NULL') . ', `empresa_id`=' . (($data['empresa_id'] !== '' && $data['empresa_id'] !== NULL) ? "'{$data['empresa_id']}'" : 'NULL') . ', `contato_id`=' . (($data['contato_id'] !== '' && $data['contato_id'] !== NULL) ? "'{$data['contato_id']}'" : 'NULL') . ', `str_gestor`=' . (($data['str_gestor'] !== '' && $data['str_gestor'] !== NULL) ? "'{$data['str_gestor']}'" : 'NULL') . ', `str_telefone`=' . (($data['str_telefone'] !== '' && $data['str_telefone'] !== NULL) ? "'{$data['str_telefone']}'" : 'NULL') . ', `str_email`=' . (($data['str_email'] !== '' && $data['str_email'] !== NULL) ? "'{$data['str_email']}'" : 'NULL') . ', `float_salario`=' . (($data['float_salario'] !== '' && $data['float_salario'] !== NULL) ? "'{$data['float_salario']}'" : 'NULL') . ', `int_maquinas`=' . (($data['int_maquinas'] !== '' && $data['int_maquinas'] !== NULL) ? "'{$data['int_maquinas']}'" : 'NULL') . ', `str_beneficios`=' . (($data['str_beneficios'] !== '' && $data['str_beneficios'] !== NULL) ? "'{$data['str_beneficios']}'" : 'NULL') . ', `bool_abertura`=' . (($data['bool_abertura'] !== '' && $data['bool_abertura'] !== NULL) ? "'{$data['bool_abertura']}'" : 'NULL') . ', `data_indicacao`=' . (($data['data_indicacao'] !== '' && $data['data_indicacao'] !== NULL) ? "'{$data['data_indicacao']}'" : 'NULL') . ', `str_descricao`=' . (($data['str_descricao'] !== '' && $data['str_descricao'] !== NULL) ? "'{$data['str_descricao']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="requerimento_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: requerimento_after_update
	if(function_exists('requerimento_after_update')){
		$res = sql("SELECT * FROM `requerimento` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!requerimento_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='requerimento' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function requerimento_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/requerimento.str_status.csv')){
		$str_status_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/requerimento.str_status.csv')));
	...
	if(is_file(dirname(__FILE__).'/hooks/requerimento.str_reposicao.csv')){
		$str_reposicao_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/requerimento.str_reposicao.csv')));
	...
	if(is_file(dirname(__FILE__).'/hooks/requerimento.bool_abertura.csv')){
		$bool_abertura_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/requerimento.bool_abertura.csv')));
	...
	if($selected_id){
		...
		$res = sql("select * from `requerimento` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'requerimento_view.php', false);
		}
			...
			$j("#empresa_id-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_empresa_id__RAND__.value, t: 'requerimento', f: 'empresa_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'requerimento', f: 'empresa_id' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#contato_id-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_empresa_id: filterer_empresa_id, id: AppGini.current_contato_id__RAND__.value, t: 'requerimento', f: 'contato_id' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_empresa_id: filterer_empresa_id, s: term, p: page, t: 'requerimento', f: 'contato_id' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#contato_id-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_contato_id__RAND__.value, t: 'requerimento', f: 'contato_id' },
					success: function(resp){
						$j('[name="contato_id"]').val(resp.results[0].id);
						$j('[id=contato_id-container-readonly__RAND__]').html('<span id="contato_id-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=tb_contato_view_parent]').hide(); }else{ $j('.btn[id=tb_contato_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/requerimento_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/requerimento_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return requerimento_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return requerimento_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return requerimento_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('requerimento');
	if($selected_id){
		$jdata = get_joined_record('requerimento', $selected_id);
		if($jdata === false) $jdata = get_defaults('requerimento');
		$rdata = $row;
	}
	$templateCode .= loadView('requerimento-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: requerimento_dv
	if(function_exists('requerimento_dv')){
		$args=array();
		requerimento_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
