...
<script>
	$j(function(){
		...
		var data = {
			...
			contrato_id: <?php echo json_encode(array('id' => $rdata['contrato_id'], 'value' => $rdata['contrato_id'], 'text' => $jdata['contrato_id'])); ?>,
		...
		/* saved value for contrato_id */
		cache.addCheck(function(u, d){
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'contrato_id' && d.id == data.contrato_id.id)
				return { results: [ data.contrato_id ], more: false, elapsed: 0.01 };
			return false;
		});
