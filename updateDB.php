<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('tb_vaga', "create table if not exists `tb_vaga` (   `id` INT unsigned not null auto_increment , primary key (`id`), `requerimento_id` INT unsigned , `int_vaga_numero` INT , `dta_abertura` DATE , `dta_previsao_fechamento` DATE , `dta_fechamento` DATE , `str_posicao` VARCHAR(150) , `empresa_id` INT unsigned , `str_alocacao` VARCHAR(40) , `recrutador_id` INT unsigned not null , `str_prioridade` VARCHAR(150) not null , `str_status` VARCHAR(150) not null , `canal_fechamento` VARCHAR(40) , `str_obs` VARCHAR(255) , `dta_inicio` DATE , `str_contratado_nome` INT unsigned ) CHARSET utf8", $silent);
		setupIndexes('tb_vaga', array('requerimento_id','empresa_id','recrutador_id','str_contratado_nome'));
