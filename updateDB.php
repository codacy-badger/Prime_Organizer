<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('tb_requerimento', "create table if not exists `tb_requerimento` (   `id` INT unsigned not null auto_increment , primary key (`id`), `dta_requisicao` DATE , `str_status` VARCHAR(40) default 'Pendente' , `dta_abertura` DATE , `dta_fechamento` DATE , `recrutador_id` INT unsigned not null , `str_posicao` VARCHAR(150) not null , `int_n_vagas` INT not null default '1' , `str_reposicao` VARCHAR(40) not null default 'N&#195;O' , `str_recurso` VARCHAR(50) , `str_horario` VARCHAR(20) not null , `empresa_id` INT unsigned not null , `str_alocacao` INT unsigned , `contato_id` INT unsigned not null , `int_maquinas` INT , `str_beneficios` VARCHAR(255) , `int_salario` INT not null , `bool_abertura` VARCHAR(40) not null , `dta_indicacao` DATE , `str_descricao` VARCHAR(255) ) CHARSET utf8", $silent);
