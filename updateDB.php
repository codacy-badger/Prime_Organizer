<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('tb_vaga', "create table if not exists `tb_vaga` (   `id` INT unsigned not null auto_increment , primary key (`id`), `requerimento_id` INT unsigned , `int_vaga_numero` INT , `dta_abertura` DATE , `dta_previsao_fechamento` DATE , `dta_fechamento` DATE , `str_status` VARCHAR(40) not null default 'Encerrada' , `str_detalhe_status` VARCHAR(40) , `recrutador_id` INT unsigned not null , `str_prioridade` VARCHAR(150) not null , `str_posicao` VARCHAR(150) , `empresa_id` INT unsigned , `str_alocacao` VARCHAR(40) , `canal_fechamento` VARCHAR(50) , `str_obs` VARCHAR(255) , `dta_inicio` DATE ) CHARSET utf8", $silent, array( " ALTER TABLE `tb_vaga` CHANGE `str_detalhe_status` `str_detalhe_status` VARCHAR(40) "));
		setupIndexes('tb_vaga', array('requerimento_id','recrutador_id','empresa_id'));
		...
		setupTable('tb_requerimento', "create table if not exists `tb_requerimento` (   `id` INT unsigned not null auto_increment , primary key (`id`), `dta_requisicao` DATE , `str_status` VARCHAR(40) default 'Pendente' , `recrutador_id` INT unsigned not null , `str_posicao` VARCHAR(150) not null , `int_n_vagas` INT not null default '1' , `str_reposicao` VARCHAR(40) not null default 'N&#195;O' , `str_recurso` VARCHAR(50) , `str_horario` VARCHAR(20) not null , `empresa_id` INT unsigned not null , `str_alocacao` VARCHAR(40) not null default 'Recurso alocado na Prime Control' , `contato_id` INT unsigned not null , `int_maquinas` INT , `str_beneficios` VARCHAR(255) , `int_salario` INT not null , `bool_abertura` VARCHAR(40) not null , `dta_indicacao` DATE , `str_descricao` VARCHAR(255) ) CHARSET utf8", $silent);
		setupIndexes('tb_requerimento', array('recrutador_id','empresa_id','contato_id'));
