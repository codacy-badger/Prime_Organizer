<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('tb_vaga', "create table if not exists `tb_vaga` (   `id` INT unsigned not null auto_increment , primary key (`id`), `requerimento_id` INT unsigned , `int_vaga_numero` INT , `dta_abertura` INT unsigned , `dta_fechamento` DATE , `empresa_id` INT unsigned , `str_alocacao` INT unsigned , `recrutador_id` INT unsigned , `str_posicao` VARCHAR(150) , `str_prioridade` VARCHAR(150) not null , `str_status` VARCHAR(150) not null , `dta_previsao_fechamento` DATE , `str_obs` TINYTEXT , `dta_inicio` DATE , `str_contratado_nome` VARCHAR(150) ) CHARSET utf8", $silent, array( " ALTER TABLE `tb_vaga` CHANGE `dta_abertura` `dta_abertura` INT unsigned "," ALTER TABLE `tb_vaga` CHANGE `dta_fechamento` `dta_fechamento` DATE "));
		setupIndexes('tb_vaga', array('requerimento_id','dta_abertura','empresa_id','str_alocacao','recrutador_id'));
		...
		setupTable('tb_requerimento', "create table if not exists `tb_requerimento` (   `id` INT unsigned not null auto_increment , primary key (`id`), `dta_abertura` DATE , `str_status` VARCHAR(40) default 'Pendente' , `str_posicao` VARCHAR(40) not null , `int_n_vagas` INT not null , `str_reposicao` VARCHAR(8) not null , `str_recurso` VARCHAR(40) , `time_horario_entrada` TIME not null , `time_horario_saida` TIME not null , `empresa_id` INT unsigned not null , `contato_id` INT unsigned not null , `str_gestor` VARCHAR(40) not null , `str_telefone` VARCHAR(40) not null , `str_email` VARCHAR(40) not null , `float_salario` FLOAT(10,2) not null , `int_maquinas` INT not null default '0' , `str_beneficios` TEXT , `bool_abertura` VARCHAR(40) not null default 'Abertura imediata' , `dta_indicacao` DATE not null , `str_descricao` TEXT ) CHARSET utf8", $silent, array( " ALTER TABLE `tb_requerimento` CHANGE `dta_abertura` `dta_abertura` DATE "," ALTER TABLE `tb_requerimento` CHANGE `dta_abertura` `dta_abertura` DATE "));
