<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('tb_vaga', "create table if not exists `tb_vaga` (   `id` INT unsigned not null auto_increment , primary key (`id`), `empresa_id` INT unsigned , `str_alocacao` INT unsigned not null , `recrutador_id` INT unsigned not null , `str_posicao` VARCHAR(150) not null , `dta_abertura` DATE not null , `qtd_vagas` SMALLINT(1) unsigned not null , `str_prioridade` VARCHAR(150) not null , `str_status` VARCHAR(150) not null , `dta_fechamento` DATE , `dta_previsao_fechamento` DATE , `str_obs` VARCHAR(255) , `dta_inicio` DATE , `str_contratado_nome` VARCHAR(150) ) CHARSET utf8", $silent);
