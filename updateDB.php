<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('tb_requerimento', "create table if not exists `tb_requerimento` (   `id` INT unsigned not null auto_increment , primary key (`id`), `dta_requisicao` DATE , `dta_abertura` DATE , `dta_fechamento` DATE , `str_status` VARCHAR(40) default 'Pendente' , `dta_previsao_fechamento` DATE , `recrutador_id` INT unsigned , `str_posicao` VARCHAR(150) not null , `int_n_vagas` INT not null , `str_reposicao` VARCHAR(8) not null , `str_recurso` VARCHAR(40) , `time_horario_entrada` TIME not null , `time_horario_saida` TIME not null , `empresa_id` INT unsigned not null , `str_alocacao` INT unsigned , `contato_id` INT unsigned not null , `str_telefone` VARCHAR(40) not null , `str_email` VARCHAR(40) not null , `float_salario` FLOAT(10,2) not null , `int_maquinas` INT not null default '0' , `str_beneficios` TEXT , `bool_abertura` VARCHAR(40) not null default 'Abertura imediata' , `dta_indicacao` DATE not null , `str_descricao` TEXT ) CHARSET utf8", $silent, array( " ALTER TABLE `tb_requerimento` CHANGE `str_posicao` `str_posicao` VARCHAR(150) not null "));
